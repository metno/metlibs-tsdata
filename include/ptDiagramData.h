...
private:
  ...
  bool makeSHC_(const int diridx, WeatherParameter& wp,
		const int level);
...
public:
  ...
  void addData(const int&,
	       ...
	       vector<miTime>&);
  // merge two datasets
  void mergeData(const int&,const int&,
		 const miTime&,vector<miTime>&,
		 const int method =0);
  ...
  void replaceData(const int,const int,const vector<miTime>,
		   vector<bool>&);
  ...
  void makeDatasets(const vector<ParId>& tempmod,const vector<ParId>& destmod,
		    const vector<ParId>& subjmod,
		    const vector<ParId>& mainmod,const vector<ParId>& xtramod,
		    const vector<time_interval>&);
  ...
  bool splitVector(const ParId& vect,
		   const ParId& comp1,
		   const ParId& comp2);
