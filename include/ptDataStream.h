...
public:
  DataStream(const miString& fname)
    : IsOpen(false), InfoIsRead(false), IsCleaned(true), DataIsRead(false),
      TimeLineIsRead(false), npar(0), npos(0) ,nmod(0),
      numTimeLines(0)
    { Name = fname; }

  virtual ~DataStream() {}

  const miString& name() { return Name; }
  bool isOpen()          { return IsOpen; }
  bool infoIsRead()      { return InfoIsRead; }
  bool dataIsRead()      { return DataIsRead; }
  bool timeLineIsRead()  { return TimeLineIsRead; }
  int numParameters()    { return npar; } 
  int numPositions()     { return npos; }
  int numModels()        { return nmod; }
  
  virtual int  findStation(const miString&)=0;  // find station index
  virtual int  findDataPar(const ParId&)=0;
  virtual void clean()=0;
  virtual void cleanParData(){parameters.clear();DataIsRead = false;}
  virtual bool openStream(ErrorFlag*)=0;
  virtual bool openStreamForWrite(ErrorFlag*)=0;
  virtual bool readData(const int posIndex, const ParId&,
			const miTime&, const miTime&,
			ErrorFlag*)=0;
  virtual bool getTimeLine(const int& index,
			   vector<miTime>& tline, vector<int>& pline,
			   ErrorFlag*)=0;
  virtual bool putTimeLine(const int& index,
			   vector<miTime>& tline, vector<int>& pline,
			   ErrorFlag*){return false;}
  virtual bool putTimeLine(TimeLine& tl, vector<int>& pline,
			   ErrorFlag*){return false;}
  virtual bool getOnePar(int, WeatherParameter&, ErrorFlag*)=0;
  virtual bool putOnePar(WeatherParameter&,ErrorFlag*){return false;}
  virtual bool getStations(vector<miPosition>&){return false;}
  ...
  virtual bool writeData(const int posIndex,      //write data to file
			 ...
			 ErrorFlag*,
			 bool complete_write,
			 bool write_submodel){return false;}
  virtual bool close() = 0;
  virtual void getTextLines(const ParId p, vector<miString>& tl){tl=textLines;}
protected:
