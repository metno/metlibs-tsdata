...
struct TimeLineItem
{
  ...
  TimeLineItem() {} // leaves the TimeLineItem in an undefined state
...
public:
  ...
  int find_equal(const miTime& tp) { 
    vector<TimeLineItem>::iterator  
      where = find(data.begin(),data.end(),TimeLineItem(tp));
  ...
  bool insert(const miTime& tp, int index=0) {
    if (index < 0 || index >= MAXTIMELINES)
      return false;
    for (vector<TimeLineItem>::iterator i = data.begin();
	 i < data.end(); ++i) {
  ...
  bool Timeline(const int& index, vector<miTime>& tline) {
    ...
    if (index>=0 && index<MAXTIMELINES){
      for (int i=0; i<data.size(); i++)
	if (data[i].flag[index]) tline.push_back(data[i].time);
      return true;
    }
    return false;
  }
  ...
  int Exist(const vector<miTime>& tline){
    if (tline.size()==0) return -1;
    int i,j,k;
    vector<int> indices;
    // check first if all timepoints exists
    for (i=0; i<tline.size(); i++){
      if ((k=find_equal(tline[i]))==-1) return -1;
      indices.push_back(k);
    }
	...
	if (i==indices[k]){
	  if (!data[i].flag[j]) break;
	  if (k<(indices.size()-1)) k++;
	} else {
	  if (data[i].flag[j]) break;
	}
      }
      if (i==data.size()) return j;
    }
    return -1;
  }
  ...
  void cleanup(vector<int>& indices){
	...
	used = used || data[i].flag[j];
      if (!used) toberemoved.push_back(data[i].time);
    }
    // remove them
    if (toberemoved.size())
      for (i=0; i<toberemoved.size(); i++){
	for (vector<TimeLineItem>::iterator p = data.begin();
	     p < data.end(); ++p) {
